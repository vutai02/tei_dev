...
namespace smooth::core::network
{
    ...
    template<typename Protocol, typename Packet>
    bool SecureSocket<Protocol, Packet>::remove_secure_context()
    {
        if (secure_context != NULL) {
            SSLContext * ptr = secure_context.release();        // pointer to no-longer-managed object
            secure_context.get_deleter() ( ptr );
            return true;
        }

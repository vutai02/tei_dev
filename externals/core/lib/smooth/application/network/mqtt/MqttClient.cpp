...
namespace smooth::application::network::mqtt
{
    ...
    void MqttClient::event(const core::network::event::ConnectionStatusEvent& event)
    {
        ...
        if (event.is_connected()) cnt_reconnect = 0;
        ...
        else if (event.get_type() == event::BaseEvent::CONNECT)
        {
            ...
            if (is_mqtts)
            {
                if (mqtts_socket)
                {
                    ...
                    if (!ca_cert_.empty()) {
                        mqtts_socket->remove_secure_context();
                        load_certificate(ca_cert_);
                    }
                }
    ...
    int MqttClient::get_cnt_reconnect()
    {
        return cnt_reconnect;
    }

    void MqttClient::reset_cnt_reconnect()
    {
        cnt_reconnect = 0;
    }
}
